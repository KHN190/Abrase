...
mod tests {
    ...
    fn alloc_native() -> myriad::NativeFn {
        Rc::new(|ctx: &mut myriad::NativeCtx<'_>, _args: &[Value]| {
    ...
    fn region_pop_reclaims_native_returned_handle() {
        ...
        assert_eq!(vm.run_module(&module), Ok(Value::from_int(99)));
        assert_eq!(vm.heap_live_count(), 0, "native-returned handle inside region must be force-freed at pop");
    }
}
